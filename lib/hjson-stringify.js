...
module.exports = function (data, opt) {
  ...
  var startsWithKeyword = new RegExp(
    "^(true|false|null)\\s*((,|\\]|\\}|#|//|/\\*).*)?$"
  );
  var meta = {
  ...
  function str(value, hasComment, noIndent, isRootObject) {
    ...
    switch (typeof value) {
      ...
      case "object":
        ...
        if (keepComments) comments = sanitizeComments(common.getComment(value));
        ...
        var saveQuoteStrings = quoteStrings,
        ...
        if (isArray) {
          ...
          for (i = 0, length = value.length; i < length; i++) {
            ...
            v = value[i];
            partial.push(
              str(v, comments ? ca : false, true) + (setsep ? separator : "")
            );
            if (cpartial) {
          ...
          if (length === 0) {
            ...
            if (comments && comments.e)
              partial.push(makeComment(comments.e[0], "\n") + eolMind);
  ...
  var res = "";
  var comments = keepComments
    ? (comments = sanitizeComments(common.getComment(data) || {}).r)
    : null;
...
function sanitizeComments(comments) {
  ...
  const c = Object.fromEntries(
    Object.entries(comments?.c || {}).filter(
